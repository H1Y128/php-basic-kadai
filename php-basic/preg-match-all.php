...
<body>
   ...
   <p>
    <?php
    echo '改行以外の1回以上繰り返されている（1文字以上の）文字が含まれいるかどうかを正規表現で検索し、検索結果を配列に代入します。<br>';
   ...
   <p>
    <?php
    echo '「あ」で始まって「り」で終わる文字列と完全に一致しているかどうかを正規表現で検索し、検索結果を配列に代入します。<br>';

    if (preg_match_all('/\Aあ.*り\z/', $str, $results)) {
        echo '>検索結果：';
        print_r($results);
    } else {
        echo '>正規表現に一致しませんでした。';
